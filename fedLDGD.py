...
def train_process(filename:str, para:list):
    ...
    mu = float(para[5])      # 正则系数
    lamda_d = float(para[6])      # 正则系数
    lamda_n = float(para[7])      # 正则系数
    pattern = int(para[8])       # 训练模式
    decayN = float(para[9])     # 正则项系数衰减
    ...
    print("  lamda_n: ", lamda_n)
    print("  pattern: ", pattern)
    print("  decay_n: ", decayN)
    print("===============================")

    """用于保存数据的文件名"""
    file_name = (
        f"P{pattern}_mu{mu}_ld{lamda_d}_ln{lamda_n}_dN{decayN}_{dataset}_{mod}_lr{lr}_d{decay}_iter{n_iter}_{filename}"
    )
