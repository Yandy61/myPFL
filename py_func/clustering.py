...
def get_grad(old_model, new_model):
    """
    返回新模型`new_model`对于旧模型`old_model`的梯度,以模型形式返回
    """
    # old_model_params = [
    #     tens.detach().cpu().numpy() for tens in list(old_model.parameters())
    # ]

    # new_model_params = [
    #     tens.detach().cpu().numpy() for tens in list(new_model.parameters())
    # ]

    # grad = [
    #     new_weights - old_weights
    #     for new_weights, old_weights in zip(
    ...
    norm = deepcopy(new_model)

    for new_layer_weigths, old_layer_weigths in zip(norm.parameters(), old_model.parameters()):
        new_layer_weigths.data.sub_(old_layer_weigths.data)
